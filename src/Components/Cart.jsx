            ...
            {cart.map((product, index) => (
                <div class="relative group my-6">
                    ...
                    <div class="relative bg-white ring-1 ring-gray-900/5 rounded-xl leading-none">
                        <div key={index} className="grid grid-cols-3 md:grid-cols-5 items-center text-center m-4 py-6">
                            <img src={product.imgMin[0]} alt="product" className="m-auto h-24 w-24" />
                            <h3 className="text-gray-400 md:text-xl">{product.name}</h3>
                            <h3 className="text-green-400 font-bold">Quantity {product.quantity}</h3>
                            
                            <button onClick={() => removeItem(product.id)} className="fondo w-24 rounded text-white p-2 mt-2 mx-auto  item-center hover:shadow-lg hover:shadow-blue-900/30 transition ease-in hover:-translate-y-1 duration-200">REMOVE</button>
                            <h3 className="text-gray-500 font-bold text-xl">${product.price}</h3>
                        </div>           
                    </div>
                </div>
            ))}
